...
module ApplicationHelper
  ...
  def user_avatar_absolute_url(user, style, base64_encoded_imgs = false)
    if user.avatar.attached?
      ...
      "data:image/svg+xml;base64,#{Base64.strict_encode64(Users::InitialsAvatarService.new(user, style).call)}"
    else
      Rails.application.routes.url_helpers.avatar_path(user, style)
    end
