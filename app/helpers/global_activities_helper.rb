...
module GlobalActivitiesHelper
  ...
  def generate_link(message_item, activity)
    ...
    when Experiment
      ...
      path = obj.archived? ? project_path(obj.project, view_mode: :archived) : list_modules_experiment_path(obj)
    when MyModule
      return current_value unless obj.navigable?

      path = if obj.archived?
               list_modules_experiment_path(obj.experiment, view_mode: :archived)
             else
