<template>
  <div class="filter-attributes">
    ...
    <div class="filter-datepicker-input">
      <DateTimePicker @change="updateDate" :data-e2e="`e2e-TP-invInventoryFilterCO-inputFrom${this.filter.column.name.replace(/[\s-]+/g, '')}`" :selectorId="`TimePicker${filter.id}`" :mode="'time'"  :defaultValue="date || fallbackDate()" />
    </div>
    <span class="between-delimiter">—</span>
    <div class="filter-datepicker-to-input">
      <DateTimePicker @change="updateDateTo" :data-e2e="`e2e-TP-invInventoryFilterCO-inputTo${this.filter.column.name.replace(/[\s-]+/g, '')}`" :selectorId="`TimePickerTo${filter.id}`" :mode="'time'" :defaultValue="dateTo || fallbackDate(7 * 24 * 60 * 60)" />
    </div>
  </div>
</template>
