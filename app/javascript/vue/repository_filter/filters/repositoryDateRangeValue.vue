<template>
  <div class="filter-attributes">
    ...
    <div class="filter-datepicker-input">
      <DateTimePicker @change="updateDate" :data-e2e="`e2e-DP-invInventoryFilterCO-inputFrom${this.filter.column.name.replace(/[\s-]+/g, '')}`" :selectorId="`DatePicker${filter.id}`" :mode="'date'" :defaultValue="date || fallbackDate()" />
    </div>
    <span class="between-delimiter">—</span>
    <div class="filter-datepicker-to-input">
      <DateTimePicker @change="updateDateTo" :data-e2e="`e2e-DP-invInventoryFilterCO-inputTo${this.filter.column.name.replace(/[\s-]+/g, '')}`" :selectorId="`DatePickerTo${filter.id}`" :mode="'date'" :defaultValue="dateTo || fallbackDate(7 * 24 * 60 * 60)" />
    </div>
  </div>
</template>
