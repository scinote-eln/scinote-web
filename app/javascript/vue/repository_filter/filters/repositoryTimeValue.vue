  ...
  <div class="filter-attributes">
    ...
    <template v-if="!isPreset">
      <div class="filter-datepicker-input">
        <DateTimePicker @change="updateDate" :data-e2e="`e2e-TP-invInventoryFilterCO-input${this.filter.column.name.replace(/[\s-]+/g, '')}`" :selectorId="`TimePicker${filter.id}`" :mode="'time'"  :defaultValue="date || fallbackDate()" />
      </div>
      <span class="between-delimiter" v-if="operator == 'between'">—</span>
      <div class="filter-datepicker-to-input" v-if="operator == 'between'">
        <DateTimePicker @change="updateDateTo" :data-e2e="`e2e-TP-invInventoryFilterCO-inputUpdate${this.filter.column.name.replace(/[\s-]+/g, '')}`" :selectorId="`TimePickerTo${filter.id}`" :mode="'time'" :defaultValue="dateTo || fallbackDate(7 * 24 * 60 * 60)" />
      </div>
    </template>
  </div>
</template>
