  ...
  <div class="filter-attributes">
    ...
    <div class="datetime-filter-attributes">
      <div class="filter-datepicker-input">
        <DateTimePicker @change="updateDate" :data-e2e="`e2e-DP-invInventoryFilterCO-inputFrom${this.filter.column.name.replace(/[\s-]+/g, '')}`" :selectorId="`DatePicker${filter.id}`"  :defaultValue="date || fallbackDate()" />
      </div>
      <div class="between-delimiter vertical"></div>
      <div class="filter-datepicker-to-input">
        <DateTimePicker @change="updateDateTo" :data-e2e="`e2e-DP-invInventoryFilterCO-inputTo${this.filter.column.name.replace(/[\s-]+/g, '')}`" :selectorId="`DatePickerTo${filter.id}`" :defaultValue="dateTo || fallbackDate(7 * 24 * 60 * 60)" />
      </div>
    </div>
  </div>
</template>
