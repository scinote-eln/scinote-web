<template>
  <div class="content-pane flexible with-grey-background">
    <div class="content-header">
      <div class="title-row">
        <h1 class="mt-0">
          {{ i18n.t('users.settings.account.team_automations.title', { team_name: teamObject?.teamName }) }}
        </h1>
      </div>
    </div>
    <div class="text-sn-dark-grey mb-4 max-w-3xl">
      <p>{{ i18n.t("users.settings.account.team_automations.description") }}</p>
    </div>
  </div>
  ...
  <div v-if="teamObject?.teamAutomationGroups">
    <div class="flex flex-col gap-8">
      <template v-for="(_subGroups, group) in teamObject?.teamAutomationGroups" :key="group">
        <div>
          <h2>{{ i18n.t(`team_automations.groups.${group}`) }}</h2>
