...
Canaid::Permissions.register_for(StepText) do
  ...
  can :lock_step_text do |user, step_text|
    Protocol.content_locking_enabled? && can_manage_protocol_draft_in_repository?(user, step_text.step.protocol)
  end
...
Canaid::Permissions.register_for(FormResponse) do
  ...
  can :lock_step_form_response do |user, form_response|
    Protocol.content_locking_enabled? && can_manage_protocol_draft_in_repository?(user, form_response.step.protocol)
  end
...
Canaid::Permissions.register_for(Checklist) do
  ...
  can :lock_step_checklist do |user, checklist|
    Protocol.content_locking_enabled? && can_manage_protocol_draft_in_repository?(user, checklist.step.protocol)
  end
...
Canaid::Permissions.register_for(Table) do
  ...
  can :lock_step_table do |user, table|
    Protocol.content_locking_enabled? && can_manage_protocol_draft_in_repository?(user, table.step.protocol)
  end
