...
module AutomationObservers
  class AllStepsCompletionObserver < BaseObserver
    def self.on_update(step, user)
      ...
      return unless (step.saved_change_to_completed? && step.completed) || (step.saved_change_to_skipped_at? && step.skipped_at)
      return unless step.protocol.in_module?

      my_module = step.protocol.my_module

      return unless my_module.my_module_status.previous_status == my_module.my_module_status_flow.initial_status && my_module.steps.where(completed: false, skipped_at: nil).none?
