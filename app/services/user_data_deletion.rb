class UserDataDeletion
    ...
    team.transaction(isolation: :serializable) do
      ...
      team.projects.each do |project|
        project.experiments.each do |experiment|
          ...
          paperclip_file_destroy(experiment)
  ...
  def self.paperclip_file_destroy(asset)
    ...
    elsif asset.class.name == 'Experiment'
      all_styles = asset.workflowimg.styles.keys.map do |key|
        asset.workflowimg.styles[key].name
      end << :original
      asset.workflowimg.clear(*all_styles)
    else
