...
module Api
  module V1
    class TaskInventoryItemsController < BaseController
      ...
      before_action :load_repository_row, only: %i(show destroy update)
      ...
      def index
        items = @task.repository_rows.where(repository_id: Repository.viewable_by_user(current_user, @team).select(:id))
      ...
      def update
        my_module_repository_row = @repository_row.my_module_repository_rows.find_by(my_module: @task)
      ...
      def destroy
        @task.my_module_repository_rows.find(repository_row: @repository_row).destroy!
      ...
      def load_repository_row
        @repository_row = @task.repository_rows.find(params.require(:id))
      end

      def check_repository_view_permissions
        raise PermissionError.new(RepositoryRow, :read_repository) unless can_read_repository?(@repository_row.repository)
      end
