class StepsController < ApplicationController
  ...
  def index
    steps = @protocol.steps
                     .left_outer_joins(:step_comments)
                     .select('COUNT(comments.id) as comments_count')
                     .select('steps.*')
                     .preload(:assets, :user, :results, :step_comments,
    ...
    render json: steps,
           ...
           completable_step: @protocol.in_repository? ? false : can_complete_my_module_steps?(@protocol.my_module),
           checkable_checklist: @protocol.in_repository? ? false : can_check_my_module_steps?(@protocol.my_module),
           uncheckable_checklist: @protocol.in_repository? ? false : can_uncheck_my_module_steps?(@protocol.my_module)
  end
