...
module ReminderRepositoryCellJoinable
  ...
  included do
    def self.reminder_repository_cells_scope(relation, user)
      reminder_scope = relation.joins( # datetime reminders
      ...
      if RepositoryStockValue.enabled?
        reminder_scope = reminder_scope.joins( # stock reminders
      ...
      reminder_scope.joins(
        ...
        "(repository_date_time_values.id IS NOT NULL"\
        "#{' OR repository_stock_values.id IS NOT NULL' if RepositoryStockValue.enabled?}"\
        ")"
      )
    end
  end
