...
class RepositoryTimeRangeValue < RepositoryDateTimeRangeValueBase
  ...
  def self.add_filter_condition(repository_rows, join_alias, filter_element)
    ...
    when 'equal_to'
      repository_rows
        .where("(#{join_alias}.start_time AT TIME ZONE 'utc' AT TIME ZONE ?)::time = (? AT TIME ZONE ?)::time AND
               (#{join_alias}.end_time AT TIME ZONE 'utc' AT TIME ZONE ?)::time = (? AT TIME ZONE ?)::time",
               timezone, Time.zone.parse(parameters['start_time']), timezone,
               timezone, Time.zone.parse(parameters['end_time']), timezone)
    when 'unequal_to'
      repository_rows
        .where.not("(#{join_alias}.start_time AT TIME ZONE 'utc' AT TIME ZONE ?)::time = (? AT TIME ZONE ?)::time AND
                   (#{join_alias}.end_time AT TIME ZONE 'utc' AT TIME ZONE ?)::time = (? AT TIME ZONE ?)::time",
                   timezone, Time.zone.parse(parameters['start_time']), timezone,
                   timezone, Time.zone.parse(parameters['end_time']), timezone)
    when 'greater_than'
      repository_rows
        .where("(#{join_alias}.start_time AT TIME ZONE 'utc' AT TIME ZONE ?)::time > (? AT TIME ZONE ?)::time",
               timezone, Time.zone.parse(parameters['end_time']), timezone)
    ...
    when 'less_than'
      repository_rows
        .where("(#{join_alias}.end_time AT TIME ZONE 'utc' AT TIME ZONE ?)::time < (? AT TIME ZONE ?)::time",
               timezone, Time.zone.parse(parameters['start_time']), timezone)
