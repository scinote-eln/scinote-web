(function(global) {
  ...
  global.copyFromClipboard = (function() {
    ...
    function _clipboardPasteModal() {
      ...
      html +=  '</button><h4 class="modal-title">' + gon.global.ASSETS_CLIPBOARD_MODAL_TITLE + '</h4>';
      html += '</div><div class="modal-body"><p><strong>' + gon.global.ASSETS_CLIPBOARD_IMG_PREVIEW + '</strong></p>';
      html += '<canvas style="border:1px solid grey;max-width:400px;max-height:300px" id="clipboardPreview" />';
      html += '<p><strong>' + gon.global.ASSETS_CLIPBOARD_FILE_NAME + '</strong></p>';
      html += '<div class="input-group">';
      html += '<input id="clipboardImageName" type="text" class="form-control" ';
      html += 'placeholder=' + gon.global.ASSETS_CLIPBOARD_FILE_NAME_PLACEHOLDER + ' aria-describedby="image-name">';
      html += '<span class="input-group-addon" id="image-name"></span></div>';
      html += '</div><div class="modal-footer">';
      html += '<button type="button" class="btn btn-default" data-dismiss="modal">' + gon.global.GENERAL_CANCEL + '</button>';
      html += '<button type="button" class="btn btn-success" data-action="addImageFormClipboard">' + gon.global.ASSETS_CLIPBOARD_ADD_IMAGE + '</button>';
  ...
  global.DragNDropSteps = (function() {
    ...
    function _uploadedAssetPreview(asset, i) {
      ...
      html += gon.global.ASSETS_DRAGNDROP_FILE_LABEL;
      html += '<div class="pull-right">';
      html += '<a data-item-id="' + i + '" href="#">';
      html += '<span class="fas fa-times"></span></a></div></div>';
      html += '<div class="panel-body">';
      html += '<label class="control-label">';
      html += gon.global.ASSETS_DRAGNDROP_FILE_LABEL + ':</label> ';
      html += truncateLongString(asset.name,
                                 gon.global.FILENAME_TRUNCATION_LENGTH);
  ...
  global.DragNDropResults = (function() {
    ...
    function validateTextSize(input) {
      if(input.value.length > gon.global.NAME_MAX_LENGTH) {
        $(input).parent().find('.dnd-error').remove();
        $(input).after("<p class='dnd-error'>" + gon.global.GENERAL_TEXT_LENGTH_TOO_LONG + "</p>");
        isValid = false;
      } else {
        $(input).parent().find('.dnd-error').remove();
        isValid = true;
    ...
    function _uploadedAssetPreview(asset, i) {
      ...
      html += gon.global.ASSETS_DRAGNDROP_FILE_LABEL + ':</label> ';
      html += truncateLongString(asset.name,gon.global.FILENAME_TRUNCATION_LENGTH);
      html += _validateFilesSize(asset);
      html += '</div></div>';
      return html;
    }
