namespace :samples_to_repository_migration do
  def prepare_repository(team, copy_num = 0)
  ...
  def prepare_text_value_custom_columns(team, repository)
  ...
  def prepare_list_value_custom_columns_with_list_items(team, repository)
  ...
  def get_sample_fields(sample_id)
  ...
  def get_assigned_sample_module(sample_id)
  ...
  task :run, [:last_id] => :environment do |_, args|
    ...
    Team.find_each(params) do |team|
      ...
      repository = prepare_repository(team)
      custom_columns = prepare_text_value_custom_columns(team, repository) +
                       prepare_list_value_custom_columns_with_list_items(
                         team,
                         repository
                       )
