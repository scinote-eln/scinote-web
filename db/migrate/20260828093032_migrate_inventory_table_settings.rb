...
class MigrateInventoryTableSettings < ActiveRecord::Migration[7.2]
  def up
    ...
    basic_columns = []
    basic_column_names.each do |name|
      basic_columns << default_column_state.call(name)
    end

    Repository.find_each do |repository|
      repository_default_state = basic_columns.dup
      repository.repository_columns.order(:id).pluck(:id).each do |column_id|
        repository_default_state << default_column_state.call("col_#{column_id}")
      end
