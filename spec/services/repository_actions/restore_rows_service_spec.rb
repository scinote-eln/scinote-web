...
describe RepositoryActions::RestoreRowsService do
  ...
  context 'when have rows' do
    it 'update rows restored_by, restored_on, archived' do
      expect { service_call }
        .to(change { row_1.reload.restored_by }
              .and(change { row_1.restored_on })
  ...
  context 'when want to restore already restored row' do
    let(:archived_item) { create :repository_row, repository: repository }
