...
RSpec.describe "Api::V2::ExperimentUserGroupAssignmentsController", type: :request do
  before :all do
    @user = create(:user)
    ...
    create_user_group_assignment(@experiment, @owner_role, @user_group)
  ...
  describe 'GET #index' do
    it 'Response with correct user group experiment assignments' do
      hash_body = nil
      get api_v2_team_project_experiment_user_group_assignments_path(team_id: @team.id, project_id: @project.id, experiment_id: @experiment.id),
          headers: @valid_headers
      expect { hash_body = json }.not_to raise_exception
      expect(hash_body[:data]).to match(
        JSON.parse(ActiveModelSerializers::SerializableResource
          .new(@experiment.user_group_assignments, each_serializer: Api::V2::UserGroupAssignmentSerializer)
          .to_json)['data']
      )
    end
    ...
    it 'When invalid request, non existing experiment' do
      hash_body = nil
      get api_v2_team_project_experiment_user_group_assignments_path(team_id: @team.id, project_id: @project.id, experiment_id: -1), headers: @valid_headers
  ...
  describe 'GET #show' do
    it 'When valid request, user can read experiment users' do
      hash_body = nil
      get api_v2_team_project_experiment_user_group_assignment_path(
        team_id: @team.id, project_id: @project.id, experiment_id: @experiment.id, id: @experiment.user_group_assignments.order(:created_at).first.id
      ), headers: @valid_headers
      expect { hash_body = json }.not_to raise_exception
      expect(hash_body[:data]).to match(
        JSON.parse(ActiveModelSerializers::SerializableResource
          .new(@experiment.user_group_assignments.order(:created_at).first, serializer: Api::V2::UserGroupAssignmentSerializer)
          .to_json)['data']
      )
    end
    ...
    it 'When invalid request, user in not member of the experiment' do
      hash_body = nil
      get api_v2_team_project_experiment_user_group_assignment_path(
        team_id: @team.id, project_id: @invalid_project.id, experiment_id: @invalid_experiment.id, id: @invalid_experiment.id
      ), headers: @valid_headers
      expect(response).to have_http_status(403)
    end
    ...
    it 'When invalid request, non existing assignment' do
      hash_body = nil
      get api_v2_team_project_experiment_user_group_assignment_path(team_id: @team.id, project_id: @project.id, experiment_id: @experiment.id, id: -1), headers: @valid_headers
      expect(response).to have_http_status(404)
    end
  end
  ...
  describe 'PATCH #update' do
    ...
    let(:action) do
      patch(
        api_v2_team_project_experiment_user_group_assignment_path(
          team_id: @project.team.id,
          project_id: @project.id,
          experiment_id: @experiment.id,
          id: @experiment.user_group_assignments.order(:created_at).last.id
        ),
    ...
    context 'when has valid params' do
      ...
      it 'returns well formated response' do
        ...
        expect(json).to match(
          JSON.parse(ActiveModelSerializers::SerializableResource
            .new(@experiment.user_group_assignments.order(:created_at).last, serializer: Api::V2::UserGroupAssignmentSerializer)
            .to_json)
        )
      end
    end
