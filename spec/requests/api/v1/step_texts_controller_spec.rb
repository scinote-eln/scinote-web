...
RSpec.describe 'Api::V1::StepTextsController', type: :request do
  before :all do
    @user = create(:user)
  ...
  describe 'GET step_texts, #index' do
    context 'when has valid params' do
      it 'renders 200' do
        get api_v1_team_project_experiment_task_protocol_step_step_texts_path(
          team_id: @team.id,
  ...
  describe 'GET step_text, #show' do
    ...
    context 'when has valid params' do
      it 'renders 200' do
        get api_v1_team_project_experiment_task_protocol_step_step_text_path(
          team_id: @team.id,
          project_id: @project.id,
          experiment_id: @experiment.id,
          task_id: @task.id,
  ...
  describe 'POST step_text, #create' do
    let(:action) do
      post(api_v1_team_project_experiment_task_protocol_step_step_texts_path(
        team_id: @team.id,
        project_id: @project.id,
        experiment_id: @experiment.id,
        task_id: @task.id,
  ...
  describe 'PATCH step_text, #update' do
    let(:step_text) { create(:step_text, step: @step) }
    let(:action) do
      patch(api_v1_team_project_experiment_task_protocol_step_step_text_path(
        team_id: @team.id,
        project_id: @project.id,
        experiment_id: @experiment.id,
        task_id: @task.id,
  ...
  describe 'DELETE step_text, #destroy' do
    let(:step_text) { create(:step_text, step: @step) }
    let(:action) do
      delete(api_v1_team_project_experiment_task_protocol_step_step_text_path(
        team_id: @team.id,
        project_id: @project.id,
        experiment_id: @experiment.id,
        task_id: @task.id,
