...
describe Api::V1::TaskUserAssignmentsController, type: :controller do
  ...
  describe 'permissions checking' do
    ...
    it_behaves_like "a controller action with permissions checking", :get, :index do
      ...
      let(:permissions) { [MyModulePermissions::USERS_READ] }
      let(:action_params) {
        {
          team_id: team.id,
    ...
    it_behaves_like "a controller action with permissions checking", :put, :update do
      let(:user_assignment) { UserAssignment.find_by(assignable: my_module) }
      let(:testable) { my_module }
      let(:permissions) { [MyModulePermissions::USERS_MANAGE] }
      let(:action_params) {
        {
          team_id: team.id,
          project_id: project.id,
