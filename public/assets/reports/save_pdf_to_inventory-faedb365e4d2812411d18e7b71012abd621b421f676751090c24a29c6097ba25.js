(function() {
  ...
  function toggleHasFileErrorMessage(value) {
    ...
    if(element.data('hasfile')) {
      $alertSection.append(
        ...
        'The selected cell already contains a file. If you would like to replace the file click Save. Replacing the file will have the following consequences: <ul><li>previous file will be permanently deleted;</li><li>new file will be added to the Inventory item.</li></ul>' +
        '</div>'
      )
    }
  }
  ...
  function initInventoryColumnSelectPicker() {
    COLUMN_PICKER =
      $('#selectInventoryColumn')
        ...
        .ajaxSelectPicker({
          ajax: {
            url: '/available_asset_type_columns',
            type: 'POST',
            dataType: 'json',
            data: function () {
              return {
  ...
  function initInventoriesSelectPicker() {
    INVENTORY_PICKER =
      $('#selectInventory')
        ...
        .ajaxSelectPicker({
          ajax: {
            url: '/reports/available_repositories',
            type: 'POST',
            dataType: 'json',
            data: function () {
              return { q: '{{{q}}}' };
  ...
  function initializeSubmitAction() {
    $('#savePDFtoInventorySubmit').off().on('click', function() {
      animateSpinner();
      $.ajax({
        url: '/reports/save_pdf_to_inventory_item',
